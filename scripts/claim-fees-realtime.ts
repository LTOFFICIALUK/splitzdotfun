...
// HTTP claim tx fetch with real-time blockhash
...
const fetchClaimTransactionsWithRealtimeBlockhash = async (body: Record<string, any>): Promise<ClaimTxApiResponse> => {
  // Get blockhash immediately before API call
  const freshBlockhash = await connection.getLatestBlockhash('confirmed');
  console.log(`🆕 Real-time blockhash: ${freshBlockhash.blockhash} (valid until ${freshBlockhash.lastValidBlockHeight})`);
  
  // Add blockhash to body
  const requestBody = { ...body, blockhash: freshBlockhash.blockhash };
  
  console.log(`📤 Request body with real-time blockhash: ${JSON.stringify(requestBody, null, 2)}`);
...
async function claimAllFeesWithRealtimeBlockhash() {
  try {
    const PRIVATE_KEY = process.env.TREASURY_PRIVATE_KEY;
    ...
    // Process each position with real-time blockhash
    for (let i = 0; i < allPositions.length; i++) {
      ...
      // Try with real-time blockhash
      let success = false;
      
      try {
        console.log(`🔄 Attempting claim with real-time blockhash...`);
        success = await tryClaimWithRealtimeBlockhash(body, keypair, connection, commitment);
      } catch (error) {
        console.log(`❌ Real-time blockhash claim failed: ${error}`);
      }
...
async function tryClaimWithRealtimeBlockhash(
  body: any, 
  keypair: Keypair, 
  ...
  try {
    // Get transaction from BAGS API with real-time blockhash
    const api = await fetchClaimTransactionsWithRealtimeBlockhash(body);
    ...
    console.error('🚨 Real-time blockhash claim failed:', error);
    return false;
  }
  return false;
}
...
async function main() {
  console.log("🔄 Starting fee claiming job with real-time blockhash...");
  const startTime = Date.now();
  
  try {
    const claimsProcessed = await claimAllFeesWithRealtimeBlockhash();
