...
const ProfilePage: React.FC = () => {
  ...
  const initiateOAuth = async (platform: string) => {
    ...
    const oauthUrls = {
      'X': `https://x.com/i/oauth2/authorize?response_type=code&client_id=${process.env.NEXT_PUBLIC_TWITTER_CLIENT_ID}&redirect_uri=${encodeURIComponent(process.env.NEXT_PUBLIC_TWITTER_REDIRECT_URI || 'https://splitz.fun/api/auth/twitter')}&scope=${encodeURIComponent('users.read')}&state=${encodeURIComponent(generateOAuthState(publicKey, codeVerifier))}&code_challenge=${codeChallenge}&code_challenge_method=S256`,
