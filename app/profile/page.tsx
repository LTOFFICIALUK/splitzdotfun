...
const ProfilePage: React.FC = () => {
  ...
  const initiateOAuth = async (platform: string) => {
    ...
    const oauthUrls = {
      ...
      'Kick': `https://kick.com/api/v1/oauth/authorize?response_type=code&client_id=${process.env.NEXT_PUBLIC_KICK_CLIENT_ID}&redirect_uri=${encodeURIComponent(process.env.NEXT_PUBLIC_KICK_REDIRECT_URI || 'https://splitz.fun/api/auth/kick')}&scope=user:read&state=${encodeURIComponent(publicKey)}`
    };
