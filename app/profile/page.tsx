...
const ProfilePage: React.FC = () => {
  ...
  const initiateOAuth = async (platform: string) => {
    ...
    const oauthUrls = {
      ...
      'Kick': `https://id.kick.com/oauth/authorize?response_type=code&client_id=${process.env.NEXT_PUBLIC_KICK_CLIENT_ID}&redirect_uri=${encodeURIComponent(process.env.NEXT_PUBLIC_KICK_REDIRECT_URI || 'https://splitz.fun/api/auth/kick')}&scope=user:read&code_challenge=${codeChallenge}&code_challenge_method=S256&state=${encodeURIComponent(generateOAuthState(publicKey, codeVerifier))}`
    };
  ...
  const handleCancel = () => {
    setIsEditing(false);
    // Reset to original data but preserve OAuth verification status
    if (originalProfile) {
      // Reload profile data to get the latest OAuth verification status
      const loadProfile = async () => {
        try {
          const profile = await getProfile(publicKey!);
          if (profile) {
          ...
          console.error('Error reloading profile on cancel:', error);
          // Fallback to original data if reload fails
          setProfileData({
